...
IFDEF __ELKS__
	push ds
	pop es  ; ES := DS. This is needed even with `wcc -r', because the code generated by the OpenWatcom C compiler (wcc) for a switch--case may ruin ES.
	xor bx, bx  ; _argv_len_plus_envp_len. Needs initialization to 0.
	xor dx, dx  ; First _argv_count, then _argv_count_plus_envp_count. Needs initialization to 0.
	mov si, [bp+10]  ; _argv.
	call execveloop1
	push dx  ; Save _argv_count value.
	mov si, [bp+12]  ; _envp.
	call execveloop1
	push dx  ; Save _argv_count_plus_envp_count.
	mov ax, dx  ; AX := _argv_count_plus_envp_count. No need to check for overflow so far, because if this overflows, DI has already overflowed, so we don't reach this check.
	add ax, ax
	js execveoverflow
	add ax, bx  ; _argv_len_plus_envp_len.
	js execveoverflow
	add ax, 6
	js execveoverflow  ; No need to jump iff stack_bytes == 0, it can't overflow this way.
	pop si  ; Restore SI := _argv_count_plus_envp_count.
	pop di  ; Restore DI := _argv_count.
	push ax  ; Save argument 3 (stack_bytes) of the SYS_execve call below.
	push ax  ; Argument of the sbrk(...) call below.
	call _sbrk  ; This doesn't ruin ES == DS.
	pop cx  ; Clean up argument of _sbrk above.
	inc ax
	jz execveerr  ; Jump iff sbrk(...) has returned -1.
	dec ax
	push ax  ; Save argument 2 (stk_ptr) of the SYS_execve call below.
	mov dx, ax  ; pip = (unsigned short*) stk_ptr;
	xchg di, ax  ; DI (stk_ptr) := sbrk(...); AX := _argv_count.
	stosw  ; *pip++ (DI) := _argv_count. This needs DS == ES.
	xchg ax, si  ; AX := _argv_count_plus_envp_count; SI := junk.
	inc ax
	inc ax
	add ax, ax
	add ax, di  ; AX += stk_ptr.
	mov bx, ax  ; pcp (BX) = stk_ptr+2*(1+_argv_count+1+_envp_count+1);
	mov si, [bp+10]  ; p (SI) := _argv.
	call execveloop2
	stosw  ; *pip++ = 0;. AX == 0. This needs DS == ES.
	mov si, [bp+12]  ; _envp.
	call execveloop2  ; Also sets AX := 0.
	stosw  ; *pip++ = 0;. AX == 0. This needs DS == ES.
	mov al, 11  ; SYS_execve.
	mov bx, [bp+8]  ; Argument 1 of SYS_execve: _path.
	pop cx  ; Argument 2 of SYS_execve: stk_ptr. It has been pushed above.
	pop dx  ; Argument 3 of SYS_execve: stack_bytes. It has been pushed above.
	call sesys  ; rv = syscall3(SYS_execve, (int) _path, (int) stk_ptr, stack_bytes);. This doesn't ruin ES == DS. It doesn't return for success.
	neg dx  ; DX := -stack_bytes.
	push dx  ; Argument of the sbrk(...) call below.
	call _sbrk  ; sbrk(-stack_bytes).  This doesn't ruin ES == DS.
	;pop cx  ; Clean up argument of _sbrk above. Not needed, we return soon.
IFDEF U_errno
	jmp short execveerr
execveoverflow:
...
execveerr:
	mov ax, -1
	mov sp, bp  ; This pops a variable number of words.
	pop bp
	pop di
	pop si
execveretonly:
	ret
execveloop1:  ; Helper function 1 for execve.
	test si, si
	je execveretonly
	cmp word ptr [si], 0
	je execveretonly
	inc dx  ; _argv_count_plus_envp_count++;
	lodsw
	xchg di, ax
	mov cx, -1
	xor ax, ax
	repne scasb  ; This needs ES == DS.
	not cx  ; CX := strlen(*p) + 1.
	add bx, cx
	js execveoverflow
	jmp execveloop1
execveloop2:  ; Helper function 2 for execve.
	xor ax, ax
	test si, si
	jz execveretonly  ; Returns to the caller of execveloop with AX == 0.
	cmp [si], ax  ; AX == 0.
	je execveretonly  ; Returns to the caller of execveloop with AX == 0.
	mov ax, bx
	sub ax, dx
	stosw  ; This needs DS == ES.
	lodsw
	push si  ; Save.
	push di  ; Save.
	xchg di, ax  ; DI := AX; AX := junk.
	mov si, di  ; SI := argument s2 of memcpy(...) below.
	xor ax, ax  ; AL := 0; AH := junk.
	mov cx, -1
	repne scasb  ; This needs ES == DS.
	not cx  ; CX := s+strlen(...).
	mov di, bx  ; Argument s1.
	rep movsb  ; memcpy(...). This needs ES == DS.
	mov bx, di
	pop di  ; Restore.
	pop si  ; Restore.
	jmp execveloop2
...
ENDIF
	sub sp, 0cH
	mov di, 0aH[bp]  ; !! Fix assembly source style.
execve1:
	cmp word ptr [di], 0
	je execve2
	inc di
