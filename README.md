...
generated code, targeting the 16-bit Intel x86 and the operating systems
...
Explanation of the commands above above:
...
* linker (a copy of OpenWatcom v2 *wlink* and a binary patcher tool to change
  the DOS MZ .exe output if *wlink* to an i86 a.out executable)
* a small C library (libc) with more than 58 functions implemented
  (custom i86 assembly code in [ssmcc.asm](ssmcc.asm)); most of this is
  original contribution by pts
* C library headers (`#include` files); this is original contribution by pts
...
* The most important limitation of the target systems supported by ssmcc is
  ...
  only 0xfff0 bytes) in each program (process).
  Memory is limited is because pointers are stored in
  ...
  up to 635 KiB of memory in a single process. ssmcc doesn't support this.
* The bundled C library (libc) has very few (more than 58) functions
  implemented. For example, most of
  stdio (including printf(...) and scanf(...)) is missing. However,
  malloc(...), realloc(...) and free(...) are included.
* ssmcc is a bit
  slower than needed, because it compiles each C file twice (in pass 1
  it just discovers the libc function dependencies). It would be faster
  if the object files were reused between the two passes.
* ssmcc is a
  ...
  decompressed) separately for each C source file.
...
The a.out executable file format for these systems is generated by asking
the OpenWatcom v2 linker to produce a small-model DOS MZ .exe program file,
and some post-processing (28 lines of Perl code) is done to convert this
file to the relevant a.out format. Actually, thanks to the versatile
configurability of the OpenWatcom v2 tools, the program is compiled with a
carefully crafted set of C compiler and linker options, thus it's necessary
to change the first 0x20 bytes only: i.e. to replace the DOS MZ .exe header
with the relevant a.out header.
...
* Most of the a.out executable header fileds are constants, except for
  ...
  CONST2 and _DATA) and a_bss (size of segment _BSS). We can get the
  ...
  in fact, we change the first 0x22 bytes of the file.
...
* The program source files are compiled twice, and the libc library files
  are omitted on purpose in the link phase of pass 1. The error
...
* This technique is not smart enough to detect unused printf(...) format
  specifiers. For example, there is no need to implement `'l'` if a *long*
  value is never passed to printf(....), and the OpenWatcom v2 C compiler
  doesn't provide the relevant hints.
...
operating systems still usable in 2026. They still run on the i86
architecture. (Minix also supports i386 and other architectures.) Thus they
provide an excellent playground for C and assembly programmers to improve
their skills of squeezing more functionality into a small code size.
